...
2. It runs a **preflight**: the implementation approach must be recorded as ADRs
   ...
   It also checks the issue's declared prerequisites (`check-prereqs.sh`) and
   fails closed when one is still open or the base is behind the trunk.
3. The skill creates a focused branch (`feat/<slug>`, etc.) off the current
   trunk — never work on `master`/`main`.
