...
- [reference/fact-freshness.md](reference/fact-freshness.md) — the freshness, invalidation and conflict contract for operative facts: which change fires which invalidator, the derived class × event matrix, what an unreadable source yields, how conflicting evidence resolves; the machine-readable form is `preferences/fact-freshness.tsv`.
