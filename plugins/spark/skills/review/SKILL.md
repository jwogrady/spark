...
6. **Capture what's durable** — `.review-notes/` is scratch: gitignored, never
   committed, regenerated by the next audit. Before moving on, file the critical
   risks and top actions as GitHub issues (paste the evidence in — file paths,
   line numbers) and fold anything else worth keeping into committed docs.
