...
- **Terminology refined by:** #477. This decision was recorded calling the flow it names *provenance promotion*. ADR-0031 reserves `provenance` for change-over-time history owned by Git and GitHub, so the flow is now **knowledge promotion** — extracting durable meaning from evidence and promoting that meaning to the authority that owns it. The wording below uses the current term throughout; the ruling itself is unchanged. The older name is left standing wherever it records history rather than states present intent: GitHub milestone #14 (`v0.17 — Provenance promotion`), the v0.17 and v0.19 release records, `docs/problem-statement.md`, and the `evaluations/provenance-promotion/` proof directory.
...
Putting that meaning in every spoke would duplicate the same durable meaning in every one of them. Putting it in operator-global knowledge would make Status26-specific architecture travel into unrelated projects. Leaving it only in chat or raw Git history would preserve evidence without preserving the adjudicated meaning.
...
- **Spark owns knowledge promotion.** Spark surfaces the classification question, gathers evidence, and routes a durable-learning candidate to the configured hub. Spark does not become the knowledge store.
...
A spoke may identify at most one canonical memory hub for this class of cross-project knowledge. The pointer is a project fact; the hub's contents are not mirrored into the spoke.
...
- **Repeat the promotion policy and the shared meaning in every spoke.** Rejected because it recreates the synchronization burden the hub exists to remove and makes cross-project meaning diverge.
...
- `knowledge` becomes the single reusable classifier/promotion path rather than duplicating promotion logic across lifecycle skills (#376).
