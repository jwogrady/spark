...
- **The caller supplies identity, not truth.** `--pr <n>` is the entire input. The repository, the exact HEAD, the bounded work unit (the issue the PR closes), its owning issue (the native parent), the authorization, the acceptance identity, the proof that the acceptance is true at that commit, the exact-HEAD reviewer verdict, the check runs, the routine/reversible classification and head freshness are all **derived from GitHub**. A caller can name which pull request; it cannot assert that any of these is satisfied.
- **Acceptance truth needed a new record, and none existed.** No canonical surface bound an issue's acceptance to an exact commit: existing mechanisms are SHA-bound but describe *review*, or acceptance-oriented but keyed to issue identity, or session-local and untracked. The narrowest truthful mechanism is a durable attestation on the PR — `<!-- spark-acceptance pr=<n> child=#N head=<sha> contract=<id> verdict=MET -->` — imitating the reviewer lane's marker grammar and reusing `spark evidence`'s `head`/`contract` invalidator names. It proves satisfaction; it never defines acceptance, which remains the human's.
- **Shape is not authorization.** A well-formed citation authorizes nothing on its own. The cited comment is read back from GitHub and must carry exactly one structured `spark-authorizes child=#N acceptance=<id>` record naming this child and binding this acceptance. Unreadable, missing, ambiguous, malformed, mismatched or unrelated evidence declines. Native hierarchy can become authority evidence only through trusted read-back, never by caller assertion.
- **Untrusted input must never impersonate trusted output.** Caller values reach the verdict text, so control-bearing input is refused before any byte is emitted and only canonical validated identities are echoed. A newline in one field had forged a `parent outcome: CLOSED and fully satisfied` line above the real disclaimer, at exit 0, in the classifier's own voice.
- **Identities are canonical.** No zero, no leading zeros: `#0585` is refused rather than normalised, so an alias cannot walk past a refusal the bare `#585` triggers.
