...
def rows_for(tree):
    ...
    for n in sorted(fns):
        ...
        rows.append((n, f, RESP_OF[n], str(body), str(parses[n]), str(len(cs)), ";".join(cs) if cs else "-",
...
#
# function <TAB> file <TAB> responsibility <TAB> body lines <TAB> parse lines <TAB> consumer count <TAB>
# consumers <TAB> verbs
"""
...
parse_after = {r[0]: int(r[4]) for r in rows_after}
parse_before = {r[0]: int(r[4]) for r in rows_before}
parse_by_file_after = collections.Counter()
parse_by_file_before = collections.Counter()
for r in rows_after: parse_by_file_after[r[1]] += int(r[4])
for r in rows_before: parse_by_file_before[r[1]] += int(r[4])
parse_fns_after = sum(1 for r in rows_after if int(r[4]) > 0)
parse_fns_before = sum(1 for r in rows_before if int(r[4]) > 0)
...
cons_after = {r[0]: int(r[5]) for r in rows_after}
...
def consumers_of(name, rows):
    for r in rows:
        if r[0] == name: return r[6].split(";") if r[6] != "-" else []
    return []
