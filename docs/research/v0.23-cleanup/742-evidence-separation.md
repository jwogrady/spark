...
every evidence artifact of the repository — 132 files, 1,045,414 bytes, 16,912 lines under
...
|---|---|---|---|
| `active-current` | 52 | 438,709 | 7,168 |
...
| `docs/research/v0.23-cleanup` | 20 | 298,791 | 3,923 |
...
link counts here. The index's readers column records **every** reader, including this unit's own suite where it genuinely reads an
artifact: an inventory that hid a real reader would be false, and provenance is what that column is for. The
*metric* is the narrower question — how much of the corpus the repository names, not how much this index names
itself — so it is computed with this unit's five files excluded by name: `docs/ops/evidence-index.tsv`,
...
`docs/research/v0.23-cleanup/tools/evidence-reads.sh`. Both halves are checked: the suite requires the index to
record itself where it reads, and requires the footprint to leave those five out. From the readers column, minus
that exclusion:
...
**29 files, 362,853 bytes** of the 132-file, 1,045,414-byte corpus are referenced by code,
...
The boundary is not asserted by this page. It is written into **#742's own issue body** — the work unit's
contract surface — under the heading *Human scope decision — 2026-09-07*, amended by `jwogrady` at
2026-09-08T00:06:23Z. That distinction is the point: a contract amended by the owner on the issue is evidence a
pull request cannot manufacture, where a decision quoted into a pull request by its own author is not. Read the
contract, not this paragraph; the sentences below are here only so the page is legible on its own.

> For #742, completion means the evidence corpus is truthfully classified and indexed, historical material is
> separated from the normal session-oriented current-truth path to the extent measured by this work unit, and the
> remaining current dependencies on historical records are explicitly identified and retained as active
> dependencies rather than hidden or falsely classified.
>
> The following remaining dependencies are **delegated to #768 under parent #729** and are not required to be
> repaired inside #742 […] This narrowing is **not a waiver** of the v0.23 cleanup requirement. #768 remains
> required work under #729, and neither #729 nor release gate #480 may claim completion until that follow-up and
> the rest of their governed acceptance are truthfully satisfied.

So the unit's acceptance is what the contract now says it is, and this page's job is to report the three
dependencies truthfully rather than to argue they are out of scope. All three move to **#768**, a sub-issue of
#729 alongside this one, and nothing here declares #729 or #480 complete.
...
Physical, over the same roots, against `cfb2880` — the commit this branch left:

| | Files | Bytes | Lines |
|---|---|---|---|
| before | 132 | 1,045,463 | 16,914 |
| after | 132 | 1,045,414 | 16,912 |
| delta | +0 | -49 | -2 |

The corpus shrank by 49 bytes, and nothing was moved or deleted to achieve it: the evidence artifacts this unit regenerates came back smaller. The file count is unchanged, which is what says the difference is content rather than removal. The reference footprint did not move: 29 files before
this unit and 29 after, because this unit's own five files are excluded as readers and no other surface
started naming evidence. The corpus shrank and the count of referenced files did not, so the share of the corpus that any surface names by path rose.
...
and `docs/research/v0.23-cleanup/tools/evidence-reads.sh` — because a metric that counted its own machinery would
report itself as the repository's dependence on history. The capture is the clearest case: it names almost every
evidence path, because recording them is its job. They are still *recorded* as readers wherever they read; the
exclusion is arithmetic on the metric, not a gap in the inventory. The two artifacts of this unit that *are* evidence — this manifest and the observation capture — sit
