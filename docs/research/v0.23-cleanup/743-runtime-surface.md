...
| `source-collection` | 86 | 94 | 1,754 | 2,326 | reads a source of truth (git, gh, the filesystem) and returns it unjudged |
...
Module count goes 3 to 4, adding `facts.sh`. The runtime holds 275 functions and 11,891 body lines, against
249 and 10,034 before: +26 functions, +1857 body lines.
...
| `plugins/spark/lib/facts.sh` | 0 | 2,397 | +2397 |

**12,166 lines before, 14,618 after (+2452)**, against
10,034 and 11,891 body lines. Both grow: this tree adds runtime rather than only redistributing it, and the body lines say so rather than being read out of the file total.
...
9,256 of 11,891 body lines. That
...
- **No cleanup-only refactor became a rewrite** — 28 function(s) added, 2 removed, +1857 body lines — the shape of an increment, not of a restructuring.
