...
| `source-collection` | 86 | 92 | 1,754 | 1,855 | reads a source of truth (git, gh, the filesystem) and returns it unjudged |
...
Module count goes 3 to 4, adding `facts.sh`. The runtime holds 262 functions and 10,507 body lines, against
249 and 10,034 before: +13 functions, +473 body lines.
...
| `plugins/spark/lib/facts.sh` | 0 | 725 | +725 |

**12,166 lines before, 12,946 after (+780)**, against
10,034 and 10,507 body lines. Both grow: this tree adds runtime rather than only redistributing it, and the body lines say so rather than being read out of the file total.
...
8,749 of 10,507 body lines. That
...
- **No cleanup-only refactor became a rewrite** — 15 function(s) added, 2 removed, +473 body lines — the shape of an increment, not of a restructuring.
