...
| `source-collection` | 86 | 93 | 1,754 | 1,955 | reads a source of truth (git, gh, the filesystem) and returns it unjudged |
...
| `evidence-authority` | 19 | 22 | 505 | 864 | decides what the evidence is admissible for |
...
Module count goes 3 to 4, adding `facts.sh`. The runtime holds 267 functions and 10,773 body lines, against
249 and 10,034 before: +18 functions, +739 body lines.
...
| `plugins/spark/lib/facts.sh` | 0 | 1,070 | +1,070 |

**12,166 lines before, 13,291 after (+1,125)**, against
10,034 and 10,773 body lines. Both grow: this tree adds runtime rather than only redistributing it, and the body lines say so rather than being read out of the file total.
...
8,855 of 10,773 body lines. That
...
- **No cleanup-only refactor became a rewrite** — 20 function(s) added, 2 removed, +739 body lines — the shape of an increment, not of a restructuring.
