...
| `domain-semantics` | 66 | 67 | 6,809 | 6,955 | owns a rule about what the facts mean |
...
Module count goes 3 to 4, adding `facts.sh`. The runtime holds 275 functions and 11,939 body lines, against
249 and 10,034 before: +26 functions, +1905 body lines.
...
**12,166 lines before, 14,666 after (+2500)**, against
10,034 and 11,939 body lines. Both grow: this tree adds runtime rather than only redistributing it, and the body lines say so rather than being read out of the file total.
...
9,304 of 11,939 body lines. That
...
| `facts_canonical` | 10 | `canonicalization` |
| `pref_get` | 10 | `source-collection` |
| `facts_load_grammars` | 9 | `source-collection` |
...
- **Every removal cites its evidence** — 2 function(s) left the runtime — `bg_json_escape`, `json_escape_out` — and 28 arrived — `__spark_memo_write`, `cmd_facts`, `facts_acceptance_fact`, `facts_canonical` and 24 more.
...
- **No cleanup-only refactor became a rewrite** — 28 function(s) added, 2 removed, +1905 body lines — the shape of an increment, not of a restructuring.
