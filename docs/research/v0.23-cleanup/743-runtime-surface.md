...
| `source-collection` | 86 | 92 | 1,754 | 1,832 | reads a source of truth (git, gh, the filesystem) and returns it unjudged |
| `canonicalization` | 45 | 49 | 631 | 662 | normalizes what was read into this repository's vocabulary |
| `domain-semantics` | 66 | 67 | 6,809 | 6,891 | owns a rule about what the facts mean |
| `evidence-authority` | 19 | 21 | 505 | 673 | decides what the evidence is admissible for |
...
Module count goes 3 to 4, adding `facts.sh`. The runtime holds 262 functions and 10,413 body lines, against
249 and 10,034 before: +13 functions, +379 body lines.
...
| `plugins/spark/lib/facts.sh` | 0 | 618 | +618 |

**12,166 lines before, 12,839 after (+673)**, against
10,034 and 10,413 body lines. Both grow: this tree adds runtime rather than only redistributing it, and the body lines say so rather than being read out of the file total.
...
8,723 of 10,413 body lines. That
...
- **No cleanup-only refactor became a rewrite** — 15 function(s) added, 2 removed, +379 body lines — the shape of an increment, not of a restructuring.
