...
| `source-collection` | 86 | 92 | 1,754 | 1,822 | reads a source of truth (git, gh, the filesystem) and returns it unjudged |
| `canonicalization` | 45 | 49 | 631 | 662 | normalizes what was read into this repository's vocabulary |
| `domain-semantics` | 66 | 67 | 6,809 | 6,884 | owns a rule about what the facts mean |
| `evidence-authority` | 19 | 21 | 505 | 658 | decides what the evidence is admissible for |
...
Module count goes 3 to 4, adding `facts.sh`. The runtime holds 262 functions and 10,381 body lines, against
249 and 10,034 before: +13 functions, +347 body lines.
...
| `plugins/spark/lib/facts.sh` | 0 | 586 | +586 |

**12,166 lines before, 12,807 after (+641)**, against
10,034 and 10,381 body lines. Both grow: this tree adds runtime rather than only redistributing it, and the body lines say so rather than being read out of the file total.
...
8,706 of 10,381 body lines. That
...
- **No cleanup-only refactor became a rewrite** — 15 function(s) added, 2 removed, +347 body lines — the shape of an increment, not of a restructuring.
