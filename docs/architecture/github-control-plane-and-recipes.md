# GitHub control plane and versioned project recipes

This document records the product architecture that separates Spark's **generic automation control plane** from the **versioned project recipes** that later feed that control plane.

It is a roadmap/architecture contract, not evidence that unreleased behavior already exists. The owning release gates and issues remain the executable product contracts:
...
v0.25 — Versioned recipes for the control plane
        feed reusable project operating patterns into v0.24
...
Spark v0.25 recipe
= versioned expectations/patterns that drive that engine
```

A workflow is only one possible kind of automation, and only one optional thing a recipe may describe.
...
A human should not have to approve the same decision twice. If project authority has already approved `P1`, a milestone, a true blocker, or a parent/sub-issue relationship, Spark's job is to project that approved truth mechanically and verify it.
...
verification             -> checks/reviews bound to the exact state they prove

release readiness        -> release-gate issue carrying the milestone scope
...
Spark should rely only on facts that are mechanically established or durably authorized.
...
- a recommendation is approval;
- a milestone or priority is authorized because it seems logical;
...
- an installed integration proves it participated in a specific work unit.

A useful permanent rule is:

> **Availability is not authority. Presence is not participation. Recommendation is not decision.**

### 2.4 Forms of automation the control plane supports

The v0.24 control plane must not be reduced to workflow execution.
...
The workflow is composed from trustworthy primitives; the workflow is not the definition of Spark.
...
```text
ASK HUMAN -> later discover the answer was already in GitHub/repository truth
```
...
- OPEN does not prove work remains;
- CLOSED does not prove the promised outcome shipped;
- repository evidence may contradict issue prose;
- an active implementation PR may already exist;
- existing durable metadata may already contain the human decision;
- missing evidence is not automatically a human judgment question.
...
Trustworthy automation needs machine contracts that do not require scraping human terminal prose.
...
The value identifies the installed/released Spark control plane that actually exercised governance. It must never be taken from the unreleased target working tree simply because that tree advertises the next version.

#710 keeps these concepts separate:

```text
Author/owner = ordinary human/project Git authority
Governor     = released Spark control plane
Execution    = role/model/provider/surface/run facts owned separately by #711
```
...
This requirement does **not** imply a new public Spark verb, GitHub App, rich card, or multi-actor execution system. The smallest deterministic helper integrated into the existing ship/projection path is sufficient if it satisfies the contract.
...
One work unit may have multiple execution records. A later reviewer must not overwrite an earlier writer record. A changed HEAD supersedes the current display while preserving historical provenance.
...
- a **manual PR-body instruction is too weak for #710** because it does not mechanically preserve the governor fact;
- a **full Spark provenance card/check is too broad for #710** because it belongs to #711.
...
## 4. v0.25: versioned project recipes

Once v0.24 provides a trustworthy control plane, v0.25 feeds reusable, version-controlled project operating patterns into that control plane.
...
recipe definition @ version
        |
        v
project authority adopts/pins recipe version
        |
        v
...
The recipe supplies **what a class of project should look like**. v0.24 supplies **how authorized state is safely realized and verified in GitHub**.

### 4.1 Recipe is broader than workflow

A recipe is a version-controlled project operating pattern.

It may define:

- project type / recipe identity;
- recipe version;
...
An infrastructure recipe may primarily define invariants, validation, authority boundaries, and recovery expectations without defining a long sequential workflow.

### 4.2 Recipe availability is not authority
...
A recipe existing in Spark or in a repository is not authority to reshape a project.

- publication makes a recipe/version available;
...
A project should be able to record a durable recipe identity such as:

```text
recipe: website.service-area-business
version: 2.3.0
```
...
Recipe migrations use the same v0.24 preview -> apply -> read-back -> verify discipline.

Once converged, a second run should be a no-op.

### 4.4 Composition and specialization

Real projects should not require copy/pasting entire recipe contracts.

The recipe layer should support deterministic composition/inheritance, or deliberately prove a smaller equivalent sufficient.
...
Conflicting recipe requirements must have deterministic precedence/conflict behavior. Silent load-order resolution is not acceptable.

### 4.5 Representative recipe classes
...
An individual tenant can compose the platform/tenant contract with a business/project recipe instead of becoming a giant one-off governance definition.

---

## 5. Relationship between control plane and recipes
...
Recipe
= versioned expectations for a class of project

Project
= an instance/composition of adopted recipes

Observed state
= what is actually true now

Reconcile
= recipe/governance expectation vs observed truth
...
Recipes provide the reusable domain-specific expectations.
...
This includes the generic mechanics for:
...
v0.24 must remain useful with **no recipe at all**. A human or another governed source may authorize state and Spark can still operate the control plane safely.

### v0.25

**Versioned recipes for the GitHub control plane.**

Package reusable project expectations so multiple project classes can drive the same released v0.24 control plane consistently.

v0.25 must not build a second GitHub governance/mutation engine. Recipes compile to expectations; v0.24 primitives realize and verify them.

### Later

Re-evaluate from field evidence after the control-plane and recipe layers are real and dogfooded. Provider-neutral execution parity is one preserved candidate, not a current v0.25 requirement.
...
> **Spark turns authorized intent into verified project truth using GitHub as the control plane. Versioned recipes tell that control plane what well-governed looks like for a particular class of project.**
...
v0.25 — feed the control plane versioned recipes
