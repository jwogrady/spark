...
- [adr/0034-fact-freshness-contract.md](adr/0034-fact-freshness-contract.md) — **Accepted**: freshness by identity — one machine-readable contract for which change invalidates which fact (a derived class × event matrix), what an unreadable source yields (UNKNOWN, never stale reuse) and how conflicting evidence resolves (CONFLICT, never arbitration); versioned with the fact model.
...
- [governance/guard-scoping-friction.md](governance/guard-scoping-friction.md) — governance friction: guards scoped by command shape over-block read-only work; the #526 cure and the outstanding harness instance
